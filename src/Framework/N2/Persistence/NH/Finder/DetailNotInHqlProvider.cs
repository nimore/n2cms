...
{
    public class DetailNotInHqlProvider<T> : AbstractInHqlProvider<T>
    {
        public DetailNotInHqlProvider(Operator op, string name, T[] values)
            : base(op, name, values)
        {
        }
        ...
        public override void AppendHql(StringBuilder from, StringBuilder where, int index)
        {
            string format = Name != null
                                ? " {0} ci in (select cd.EnclosingItem from ContentDetail cd where cd.{1} not in ({2}) AND cd.Name = :{3})"
                                : " {0} ci in (select cd.EnclosingItem from ContentDetail cd where cd.{1} not in ({2}))";
