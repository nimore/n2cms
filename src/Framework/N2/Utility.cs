...
namespace N2
{
	...
	public static class Utility
	{
		...
		private static readonly Regex StripTagsExpression = new Regex("<!*[^<>]*>", RegexOptions.Multiline);
					...
					catch (InvalidCastException)
					{
						if (value is string)
						{
							var parseMethod = destinationType.GetMethods(BindingFlags.Static | BindingFlags.Public)
								...
								.FirstOrDefault();
							if ((parseMethod != null) && (parseMethod.GetParameters().Length == 1) &&
		...
		/// <summary>
		///     Tries to find a property matching the supplied expression, returns null if no property is found with the first
		...
		public static void SetProperty(object instance, string propertyName, object value)
		{
			...
			if (!pi.CanWrite)
				throw new N2Exception("The property '{0}' on the instance of type '{1}' does not have a setter.", propertyName,
					instanceType);
		...
		public static IEnumerable<ContentItem> UpdateSortOrder(IEnumerable<ContentItem> siblings)
		{
			...
			var sortOrderBeforeLast = int.MinValue;
			ContentItem last = null;
			var lastSortOrderChanged = false;
			var index = 0;
			foreach (var current in siblings)
			{
				if (current.SortOrder <= lastSortOrder)
				{
					var gapBeforeLast = (int) Math.Min((long) lastSortOrder - sortOrderBeforeLast, int.MaxValue);
					var gapBeforeCurrent = (int) Math.Min((long) current.SortOrder - sortOrderBeforeLast, int.MaxValue);
		...
		/// <remarks>
		///     To persist the new ordering one should call <see cref="Utility.UpdateSortOrder" /> and save the returned
		///     items. If the items returned from the <see cref="ContentItem.GetChildren()" /> are moved with this method the
		///     changes will not be persisted since this is a new list instance.
		/// </remarks>
		...
		{
			return Insert(item, newParent, new ItemComparer(sortExpression));
		}
		...
		public static string GetGlobalResourceString(string classKey, string resourceKey)
		{
			if ((classKey != null) && (resourceKey != null) && (HttpContext.Current != null))
			{
				...
				var key = new ResourceKey(classKey, resourceKey, cultureName);
				try
				{
		...
		public static IEnumerable<string> ListFiles(string physicalBasePath, string filter, bool recursive = true)
		{
			...
			{
				if (Directory.Exists(toExplore[0]))
				{
					files.AddRange(Directory.GetFiles(toExplore[0], filter));
					if (recursive)
						toExplore.AddRange(Directory.GetDirectories(toExplore[0]));
				}
		...
		public static IDisposable TimeCapsule(DateTime frozenTime)
		{
			...
			return new Scope(() => CurrentTime = previous);
		}
		...
		public static void InvokeEvent(EventHandler<CancellableItemEventArgs> preHandlers, ContentItem item, object sender,
			Action<ContentItem> finalAction, EventHandler<ItemEventArgs> postHandlers)
		{
		...
		public static ContentItem InvokeEvent(EventHandler<CancellableDestinationEventArgs> handler, object sender,
			ContentItem source, ContentItem destination, Func<ContentItem, ContentItem, ContentItem> finalAction,
			EventHandler<DestinationEventArgs> postHandler)
		{
		...
		public static string GetTrail(this ContentItem item)
		{
			...
			var ancestralTrail = item.AncestralTrail ?? GetTrail(item.Parent);
		...
		public static string ExtractFirstSentences(string html, int maxLength)
		{
			...
			for (var i = 0; (i < html.Length) && (i < maxLength); i++)
				switch (html[i])
				{
					case '.':
					case '!':
		...
		public static string GetFileSizeString(long p, bool base10)
		{
			...
			{
				if (p >= 1000000)
					return string.Format("{0:N1} MB", p/1000000.0);
				if (p >= 4000)
					return string.Format("{0:N1} KB", p/1000.0);
				return string.Format("{0:N0} bytes", (double) p);
			}
			if (p >= 1048576)
				return string.Format("{0:N1} MiB", p/1048576.0);
			if (p >= 4096)
				return string.Format("{0:N1} KiB", p/1024.0);
			return string.Format("{0:N0} bytes", (double) p);
		}
		...
		internal static IEngine Engine(this Page page)
		{
		...
		public static bool IsPublished(this ContentItem item)
		{
			...
			switch (item.State)
			{
				...
				case ContentState.Published:
					return item.Published.HasValue && (item.Published <= CurrentTime())
					       && (!item.Expires.HasValue || (CurrentTime() < item.Expires.Value));
				default:
					return false;
			}
		}
		...
		public static bool IsExpired(this ContentItem item)
		{
			...
			return (item.State == ContentState.Unpublished) || (item.Expires.HasValue && (item.Expires.Value < CurrentTime()));
		}

		public static bool SetPropertyOrDetail(ContentItem item, string detailName, object value)
		{
			var existing = item[detailName];
		...
		{
			return Regex.Replace(format, "{([\\w\\.]+)}", m => (string) Evaluate(values, m.Groups[1].Value),
		...
		internal static List<INameable> FindEmpty(ContentItem item)
		{
			...
			foreach (var detail in item.Details)
				if ((detail.EnclosingItem.ID == 0) && !ReferenceEquals(detail.EnclosingItem, item))
					list.Add(detail);
		...
		public static ItemList<T> WhereNavigatable<T>(this IContentList<T> items, IPrincipal byUser = null,
		...
		private class ContentDictionary : IDictionary<string, object>
		{
			...
			public ICollection<string> Keys
			{
				...
				{
					return
						ContentItem.KnownProperties.WritableProperties.Union(item.Details.Select(d => d.Name))
							.Union(item.DetailCollections.Select(dc => dc.Name))
							.ToList();
				}
			}
			...
			{
				return
					ContentItem.KnownProperties.WritableProperties.Select(wp => new KeyValuePair<string, object>(wp, WriteProperty(wp)))
						.Union(item.Details.Select(d => new KeyValuePair<string, object>(d.Name, WriteDetailValue(d))))
						.Union(
							item.DetailCollections.Select(
								dc => new KeyValuePair<string, object>(dc.Name, dc.Details.Select(d => WriteDetailValue(d)).ToList())))
						.ToList()
						.GetEnumerator();
			}
			...
			private object WriteDetailValue(ContentDetail d)
			{
